...
const calculateSavings = () => {
  ...
  const percentageValue = document.getElementById("save").value;
  const savePercentage = parseInt(percentageValue);

  //   Validate saving percentage value
  if (savePercentage < 0) {
    alert("Provide positive saving value");
  }
  const savingAmountValue = ((savePercentage * parseInt(income) / 100));
  const savingAmount = parseFloat(savingAmountValue);

  // calculate remaining balance
  const balanceValue = document.getElementById("balance").innerText;
  const balance = parseFloat(balanceValue);
  const remainingBalance = parseInt(balance) - parseFloat(savingAmount);
  ...
  if (savingAmount > balance) {
    ...
    document.getElementById("remaining-balance").innerText = parseFloat(remainingBalance);
  }
};
